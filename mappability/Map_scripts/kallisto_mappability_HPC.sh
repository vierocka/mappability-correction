...
#SBATCH --output=kallisto_HPC_%j.out
#SBATCH --mail-type=BEGIN,END,FAIL

# ─────────────────────────────────────────────────────────────────────────────
# Kallisto cross-validation — HPC-specific script with absolute paths.
#
# Reads three sources against two indices:
#   MANE spliced RNA reads  (SE+PE, 75/100/150 bp) → reused from v5 STAR results
#   Exon-flank reads        (SE 75/100/150 bp)      → reused from v5 STAR results
#   Dedup CDS reads         (SE+PE, 75/100/150 bp)  → simulated here (or reused from v7)
#
# Two Kallisto indices:
#   MANE RNA   — 19,437 full spliced transcripts (UTRs included)
#   Dedup CDS  — 93,088 CDS sequences (no UTRs)
#
# Key comparisons:
#   MANE reads vs MANE index   → quantifies inflation bias at HLA locus
#   MANE reads vs dedup CDS    → measures UTR smoothing contribution
#   Dedup CDS reads vs both    → CDS-level uniqueness; coding redundancy
#   Exon-flank reads vs both   → intron retention proxy; expected to fail
# ─────────────────────────────────────────────────────────────────────────────
...
echo "════════════════════════════════════════════════════════"
echo "Kallisto cross-validation — HPC"
echo "Started : $(date)"
echo "Host    : $(hostname)"
echo "BASE    : $BASE"
echo "════════════════════════════════════════════════════════"
...
# ── Step 1: Simulate dedup CDS reads ─────────────────────────────────────────
# Reused from v7 STAR scripts if already present; otherwise generated here.

echo ""
echo "Step 1: Checking / generating dedup CDS reads..."
...
simulate_cds() {
    ...
    if [ -f "$outdir/simreads/sim_R1.fastq" ]; then
        echo "  Reads exist: $(basename $outdir)"
        return
    fi
...
r2 = open(simdir / 'sim_R2.fastq', 'w') if mode == 'PE' else None

with open(simdir / 'sim_R1.fastq', 'w') as r1:
    for seq_id, seq in parse_fasta(cds_fa):
        L = len(seq)
        if L < rl: n_skip += 1; continue
        n_seq += 1
        for pos in range(L - rl + 1):
            fwd = seq[pos : pos + rl]
            if fwd.count('N') > rl // 5: continue
            r1.write(f'@{seq_id}|{pos}\n{fwd}\n+\n{qual}\n')
            if r2: r2.write(f'@{seq_id}|{pos}\n{revcomp(fwd)}\n+\n{qual}\n')
            n_reads += 1

if r2: r2.close()
print(f"  Sequences: {n_seq:,}  (too short: {n_skip})  Reads: {n_reads:,}", flush=True)
PYEOF
}
...
run_pe() {
    ...
    if [ ! -f "$r1" ] || [ ! -f "$r2" ]; then
        echo "  WARNING: reads not found — skipping $(basename $outdir)."
        return
    fi
...
# ── Dedup CDS reads — SE (CDS uniqueness / coding redundancy) ────────────────

echo ""
echo "━━━━ Dedup CDS reads — SE (CDS uniqueness) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo ""
echo "━━━━ Dedup CDS reads — PE (CDS uniqueness) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
