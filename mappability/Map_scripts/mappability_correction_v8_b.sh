#!/bin/bash
# v8_b — MANE RNA reads | proper PE simulation | 150 bp | dedup CDS reference
#
# Reuses reads from v8_a (SIM_DIR). Aligns against dedup CDS (93,088 seqs).
# Compare to v8_a (full genome) to isolate the competitor-set effect on UF
# when using realistic PE geometry.
...
OUTDIR="$SCRIPT_DIR/../results/mappability_dedup_cds_RNA_PE_L150bp_proper"
THREADS=64

mkdir -p "$OUTDIR/bam" "$OUTDIR/logs"

echo "════════════════════════════════════════════════════════"
echo "Mappability — v8_b (MANE RNA reads, proper PE, 150 bp, dedup CDS)"
echo "Reads from: $SIM_DIR"
echo "════════════════════════════════════════════════════════"

# ── Step 0: STAR index (dedup CDS) ───────────────────────────────────────────

echo ""
echo "Step 0: Checking STAR dedup CDS index..."
mkdir -p "$CDS_IDX"
...
echo ""
echo "Step 2: STAR alignment (PE, dedup CDS, $THREADS threads)..."
...
echo "════════════════════════════════════════════════════════"
echo "Done: $OUTDIR/transcript_uniqueness_factors_dedup_cds_RNA_PE_L150bp_proper.tsv"
echo "════════════════════════════════════════════════════════"
